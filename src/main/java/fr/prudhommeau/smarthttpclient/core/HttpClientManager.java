...
public class HttpClientManager {
    ...
    public HttpClientManager(Object initiator) {
        ...
        PlainConnectionSocketFactory socksConnectionManagerHttpConnectionSocketFactory = new HttpClientManagerSocketFactory.HttpClientManagerPlainConnectionSocketFactory();
        SSLConnectionSocketFactory socksConnectionManagerHttpsConnectionSocketFactory = new HttpClientManagerSocketFactory.HttpClientManagerSSLConnectionSocketFactory();
            ...
            while (!smartThreadPool.isInterrupted()) {
                ...
                try {
                    ...
                    for (HttpRequestThread httpRequestThread : threadToInterruptForResponseTimeThresholdList) {
                        ...
                        logger.debug("Stopping HTTP request thread because HTTP response exceeded " + requestTimeoutInMilliseconds + " milliseconds - httpRequestThread [" + httpRequestThread + "]");
                    }
                    for (HttpRequestThread httpRequestThread : threadToInterruptForReadTimeThresholdList) {
                        ...
                        httpRequestThread.executeHttpRequestThreadRetryStrategy(exception);
                        logger.debug("Stopping HTTP request thread because HTTP response read exceeded " + requestTimeoutInMilliseconds + " milliseconds - httpRequestThread [" + httpRequestThread + "]");
                    }
                } finally {
                    lock.unlock();
                }
